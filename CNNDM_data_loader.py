...
def get_SM_loader(df, n_data_epoch=None, **kwargs):
    ...
    if n_data_epoch is not None:
        ...
        sampler = RandomSampler(
            dataset, replacement=True, num_samples=n_data_epoch)
    ...
    def sm_collate_fn(batch):
        ...
        target_WORD_encoding = batch.target_WORD_encoding.apply(
            torch.LongTensor)
        target_WORD_encoding = pad_sequence(
            target_WORD_encoding, batch_first=True, padding_value=0)

        source_WORD_encoding = batch.source_WORD_encoding.apply(
            torch.LongTensor)
        source_WORD_encoding = pad_sequence(
            source_WORD_encoding, batch_first=True, padding_value=0)

        source_WORD_encoding_extended = batch.source_WORD_encoding_extended.apply(
            torch.LongTensor)
        ...
        focus_segment_mask = batch.focus_segment_mask.apply(torch.LongTensor)
        focus_segment_mask = pad_sequence(
            focus_segment_mask, batch_first=True, padding_value=0)
    ...
    if sampler is not None:
        if kwargs['shuffle'] == True:
            ...
            return DataLoader(dataset, collate_fn=sm_collate_fn_val, sampler=sampler, **kwargs)
    else:
        if kwargs['shuffle'] == True:
