...
def get_loader(config, data_dir,VAL_SAMPLE = 10000, get_test = False):
    if config.task == 'QG':
        ...
        data_dir = current_dir.joinpath('cnndm_segment_out/')
        with open(data_dir.joinpath('vocab.pkl'), 'rb') as f:
            word2id, id2word = pickle.load(f)
    ...
    if get_test:
        ...
        return train_loader, val_loader, word2id, id2word
