...
class FocusedEncoder(nn.Module):
    def __init__(self, word_embed_size=300, answer_position_embed_size=16, ner_embed_size=16, pos_embed_size=16,
        ...
        if use_focus:
            rnn_input_size += focus_embed_size * 2
    ...
    def forward(self,
        ...
        if self.use_focus:
            ...
            focus_segment_mask = focus_segment_mask[idx_sorted]
            # # Focus Dropout
            # if self.training:
            ...
            focus_segment_embedding = self.focus_segment_embed(focus_segment_mask)
            enc_input = torch.cat([enc_input, focus_embedding,focus_segment_embedding], dim=2)
