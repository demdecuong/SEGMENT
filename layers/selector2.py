...
class ParallelDecoder(nn.Module):
    def __init__(self, embed_size=300, enc_hidden_size=512, dec_hidden_size=512,
        ...
        self.out_mlp = nn.Sequential(
            # nn.Linear(enc_hidden_size + embed_size , dec_hidden_size),
            nn.Linear(embed_size * 2, dec_hidden_size),
            nn.ReLU(),
    ...
    def forward(self,
        ...
        # [B, max_source_len, enc_hidden_size + embed_size]
...
class ParallelSelector(nn.Module):
    ...
    def __init__(self,
        ...
        self.encoder = EncoderWrapper(word_embed_size,
                                            n_layers = 4,
                                            n_head = 8,
                                            d_k = word_embed_size//8,
                                            d_v = word_embed_size//8,
    ...
    def forward(self,
        ...
        enc_outputs , _  = self.encoder(source_WORD_encoding,None,return_attns = False)

        # enc_outputs2, h = self.encoder(
        #     source_WORD_encoding,
        #     answer_position_BIO_encoding,
        #     ner_encoding,
        #     pos_encoding,
        #     case_encoding)
        # h = torch.mean(h[0],dim = 1)
        ...
        if mixture_id is None:
            ...
            mixture_id = torch.arange(self.n_mixture, dtype=torch.long,
                                      device=enc_outputs.device).unsqueeze(0).repeat(B, 1).flatten()
        else:
