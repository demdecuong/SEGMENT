...
from utils.accuracy import cal_performance
...
if __name__ == "__main__":
    ...
    if config.task == 'QG':
        ...
        # DATASET_PATH = './gigaword_out'
        DATASET_PATH = './cnndm_segment_out'
    ...
    print("Loading data...")
    train_loader, val_loader, word2id, id2word = get_loader(config, data_dir, get_test = False)

    PAD_ID, UNK_ID = word2id['<pad>'], word2id['<unk>']
    vocab_size = len(word2id)
    print('Loaded data loaders & Vocab!')
    print('Vocab Size:', vocab_size)
    print(config.load_ckpt)
    model = build_model(config, word2id, id2word)
    if config.load_ckpt != -1:
    ...
    for epoch in range(config.epochs):
        ...
        epoch_nll_loss = []
        epoch_focus_loss = []
        epoch_focus_segment_loss = []
        epoch_cov_loss = []

        temp_nll_losses = []
        temp_focus_losses = []
        temp_focus_segment_losses = []
        temp_cov_losses = []
        ...
        for batch_i, batch in enumerate(train_loader):
            ...
            # We dont need : source_len, target_len, source_WORD , target_WORD , focus_WORD, oovs
            elif config.task == 'SM':
            ...
            if config.use_focus and not config.eval_focus_oracle:
                ...
                if config.n_mixture == 1:
                    ...
                    with torch.no_grad():
                        ...
                        segment_repeated_target = repeat(focus_segment_mask.float(), config.n_mixture)
                        ...
                        # [B * n_mixture, L]
                        focus_segment_loss = F.binary_cross_entropy_with_logits(
                            focus_segment_logit,
                            repeated_target,
                            reduction='none').view(B, config.n_mixture, L)
                        mixture_id = focus_segment_loss.masked_fill(
                            pad_mask, 0).sum(dim=2).argmin(dim=1)
                ...
                focus_segment_loss = F.binary_cross_entropy_with_logits(
                    focus_segment_logit,
                    focus_segment_mask.float(),
                    reduction='none')

                focus_segment_loss.masked_fill_(pad_mask, 0)
                focus_segment_loss = focus_segment_loss.sum(dim=1) / focus_len
                focus_segment_loss = focus_segment_loss.mean()
            else:
            ...
            loss = nll_loss + focus_loss + focus_segment_loss + cov_loss * config.coverage_lambda
            optimizer.zero_grad()
            loss.backward()
            ...
            if batch_i % 100 == 0 or (batch_i + 1) == n_iter:
                ...
                if config.use_focus:
                    ...
                    log_str += f' | Focus Segment Loss : {np.mean(temp_focus_segment_losses):.3f}'
                if config.model == 'PG':
